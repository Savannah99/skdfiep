"""TeCNO native training on AutoLaparo/HeiChole, with validation-set-
selected baseline and checkpoint saving.
usage: python grid_tecno_native_valselect_multi.py <dataset> <window> <top_k> <seed>
"""
import sys, random
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_tecno_{DATASET}_w{WINDOW}_topk{TOP_K}_seed{SEED}.pt")
...
print(f"RESULT tecno_native_valselect_{DATASET} window={WINDOW} topk={TOP_K} seed={SEED}  winner={winner} test_valsel={test_valsel:.4f} test_leaky={test_leaky:.4f} test_TSplus={test_tsplus:.4f}  (val: random={val_random:.4f} fixed0={val_fixed0:.4f} fixed1={val_fixed1:.4f})")
