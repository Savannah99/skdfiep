"""Trans-SVNet native training on AutoLaparo/HeiChole, with validation-set-
selected baseline and checkpoint saving. Uses the frozen TeCNO backbone
trained by train_frozen_tecno_multi.py for the same (dataset, window).
usage: python grid_transsvnet_native_valselect_multi.py <dataset> <window> <top_k> <seed>
"""
import sys, random
...
TECNO_CKPT = Path(f"/tmp/tecno_frozen_{DATASET}_w{WINDOW}.pth")
DEVICE = "cuda"
NUM_CLASSES, D_FEAT, EPOCHS = 7, 384, 50
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_transsvnet_{DATASET}_w{WINDOW}_topk{TOP_K}_seed{SEED}.pt")
...
print(f"RESULT transsvnet_native_valselect_{DATASET} window={WINDOW} topk={TOP_K} seed={SEED}  winner={winner} test_valsel={test_valsel:.4f} test_leaky={test_leaky:.4f} test_TSplus={test_tsplus:.4f}  (val: random={val_random:.4f} fixed0={val_fixed0:.4f} fixed1={val_fixed1:.4f})")
