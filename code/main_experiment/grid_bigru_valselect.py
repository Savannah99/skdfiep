"""BiGRU crossover (train=random) on Cholec80, with validation-set-selected
baseline (not test-set-selected) and checkpoint saving.
usage: python grid_bigru_valselect.py <window> <top_k> <seed>
"""
import sys, random
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_bigru_cholec80_w{WINDOW}_topk{TOP_K}_seed{SEED}.pt")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for vid in random.sample(train_ids, len(train_ids)):
        meta_lab = all_labels_meta[vid]
        if len(meta_lab) == 0: continue
        meta_feat = rand_feats(all_raw_feats[vid], rng)
        x = torch.from_numpy(meta_feat).float().unsqueeze(0).to(DEVICE)
        ...
        opt.step()
    val_jacc = eval_pool(model, val_ids, lambda vid: rand_feats(all_raw_feats[vid], rng), n_draws=N_EVAL_DRAWS)
    sched.step(val_jacc)
...
torch.save({"best_state": best_state, "seed": SEED, "window": WINDOW, "top_k": TOP_K}, CKPT_OUT)

bin_size = WINDOW // TOP_K
# validation-set selection among {random, fixed0, fixed1}
...
print(f"RESULT bigru_valselect window={WINDOW} topk={TOP_K} seed={SEED}  winner={winner} test_valsel={test_valsel:.4f} test_leaky={test_leaky:.4f} test_TSplus={test_tsplus:.4f}  (val: random={val_random:.4f} fixed0={val_fixed0:.4f} fixed1={val_fixed1:.4f})")
