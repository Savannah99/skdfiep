"""BiGRU trained NATIVELY on MobileNet-TS+ (train=infer=argmax on the
MobileNet score), completing the crossover-vs-native 2x3 matrix across all
three architectures. Validation-set-selected baseline, checkpoint saved.
usage: python grid_bigru_native_valselect.py <dataset> <window> <top_k> <seed>
"""
import sys, random
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_bigru_native_{DATASET}_w{WINDOW}_topk{TOP_K}_seed{SEED}.pt")
...
torch.save({"best_state": best_state, "seed": SEED, "window": WINDOW, "top_k": TOP_K, "train_mode": "native"}, CKPT_OUT)
...
print(f"RESULT bigru_native_valselect_{DATASET} window={WINDOW} topk={TOP_K} seed={SEED}  winner={winner} test_valsel={test_valsel:.4f} test_TSplus={test_tsplus:.4f}  (val: random={val_random:.4f} fixed0={val_fixed0:.4f} fixed1={val_fixed1:.4f})")
