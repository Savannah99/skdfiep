"""Trans-SVNet trained NATIVELY on MGSampler's or SCSampler's own selection
rule (matching the training/inference protocol already used for
MobileNet-TS+ on TeCNO/Trans-SVNet), with validation-set-selected baseline
and checkpoint saving. Uses the frozen TeCNO backbone already trained by
train_frozen_tecno_multi.py (cross-dataset) / the Cholec80 mstcn checkpoint.
usage: python grid_transsvnet_native_mgsc.py <method> <dataset> <window> <top_k> <seed>
  method in {mg, sc}; dataset in {cholec80, autolaparo, heichole}
"""
import sys, random
...
METHOD = sys.argv[1]; DATASET = sys.argv[2]; WINDOW = int(sys.argv[3]); TOP_K = int(sys.argv[4]); SEED = int(sys.argv[5])
assert METHOD in ("mg", "sc")
STRIDE = WINDOW
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_transsvnet_{METHOD}native_{DATASET}_w{WINDOW}_topk{TOP_K}_seed{SEED}.pt")
...
all_flow_raw = {v: load_flow(v) for v in train_ids + val_ids + test_ids}

all_sc_raw = {v: load_scsampler(v) for v in train_ids + val_ids + test_ids}
...
if METHOD == "mg":
    method_pool_fn = lambda vid: mgsampler_feats(all_raw_feats[vid], all_flow_raw[vid], WINDOW, STRIDE, TOP_K)
else:
    method_pool_fn = lambda vid: argmax_feats(all_raw_feats[vid], all_sc_z[vid])
...
for epoch in range(1, EPOCHS + 1):
    ...
    for vid in random.sample(train_ids, len(train_ids)):
        ...
        opt.step()
    val_jacc = eval_pool(trans_model, val_ids, method_pool_fn)
    if val_jacc > best_jacc:
        best_jacc = val_jacc
        best_state = {k: v.clone() for k, v in trans_model.state_dict().items()}

trans_model.load_state_dict(best_state)
torch.save({"model_state": best_state, "seed": SEED, "window": WINDOW, "top_k": TOP_K, "method": METHOD}, CKPT_OUT)
...
test_method = eval_pool(trans_model, test_ids, method_pool_fn)

print(f"RESULT transsvnet_{METHOD}native_{DATASET} window={WINDOW} topk={TOP_K} seed={SEED}  winner={winner} test_valsel={test_valsel:.4f} test_{METHOD.upper()}={test_method:.4f}  (val: random={val_random:.4f} fixed0={val_fixed0:.4f} fixed1={val_fixed1:.4f})")
