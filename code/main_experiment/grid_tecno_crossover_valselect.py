"""TeCNO trained under CROSSOVER (train=random resampling, infer=MobileNet-TS+
argmax), completing the crossover-vs-native 2x3 matrix across all three
architectures. Validation-set-selected baseline, checkpoint saved.
usage: python grid_tecno_crossover_valselect.py <dataset> <window> <top_k> <seed>
"""
import sys, random
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_tecno_crossover_{DATASET}_w{WINDOW}_topk{TOP_K}_seed{SEED}.pt")
...
print(f"RESULT tecno_crossover_valselect_{DATASET} window={WINDOW} topk={TOP_K} seed={SEED}  winner={winner} test_valsel={test_valsel:.4f} test_TSplus={test_tsplus:.4f}  (val: random={val_random:.4f} fixed0={val_fixed0:.4f} fixed1={val_fixed1:.4f})")
