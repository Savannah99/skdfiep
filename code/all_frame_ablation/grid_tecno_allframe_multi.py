"""TeCNO trained on ALL frames in the window across all 3 datasets, evaluated
at inference under every valid top_k for the given window, under 7 selection
strategies: random, fixed0, fixed1(mid), MobileNet-TS+ (argmax), MGSampler
(deterministic), SCSampler-style (argmax + stochastic).
usage: python grid_tecno_allframe_multi.py <dataset> <window> <seed>
"""
import sys, random
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_tecno_allframe_{DATASET}_w{WINDOW}_seed{SEED}.pt")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for vid in train_ids:
        ...
        optimizer.step()
    val_jacc = eval_pool(model, val_ids, lambda vid, i: train_pool_fn(vid))
...
for top_k in DIVISORS:
    ...
    print(f"RESULT tecno_allframe_{DATASET} window={WINDOW} topk={top_k} seed={SEED}  random={test_random:.4f} fixed0={test_fixed0:.4f} fixed1={test_fixed1:.4f} TSplus={test_tsplus:.4f} MGSampler={test_mg:.4f} SCSampler_argmax={test_sc_a:.4f} SCSampler_stoch={test_sc_s:.4f}")
