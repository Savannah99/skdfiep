"""Trans-SVNet trained on ALL frames in the window (frozen TeCNO backbone
also all-frame) across all 3 datasets, evaluated at inference under every
valid top_k for the given window, under 7 selection strategies: random,
fixed0, fixed1(mid), MobileNet-TS+ (argmax), MGSampler (deterministic),
SCSampler-style (argmax + stochastic).
usage: python grid_transsvnet_allframe_multi.py <dataset> <window> <seed>
"""
import sys, random
...
PATHS = {
    ...
    "autolaparo": dict(feat_root="/mnt/ssd4t/SurgKeyMAE/features/autolaparo/vit_small",
                        ...
                        split_dir=f"{BASE}/autolaparo/splits", tecno_ckpt=f"/tmp/tecno_frozen_autolaparo_w{WINDOW}.pth"),
    "heichole": dict(feat_root="/mnt/ssd4t/SurgKeyMAE/features/heichole/vit_small",
                      ...
                      split_dir=f"{BASE}/heichole/splits", tecno_ckpt=f"/tmp/tecno_frozen_heichole_w{WINDOW}.pth"),
}[DATASET]
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_transsvnet_allframe_{DATASET}_w{WINDOW}_seed{SEED}.pt")
...
torch.save({"model_state": best_state, "seed": SEED, "window": WINDOW, "train_mode": "allframe"}, CKPT_OUT)

for top_k in DIVISORS:
    rng_base = SEED * 100 + top_k
    test_random = eval_pool(trans_model, test_ids, lambda vid, i: rand_feats(all_raw_feats[vid], top_k, np.random.default_rng(1000+rng_base*10+i)), n_draws=N_EVAL_DRAWS)
    test_fixed0 = eval_pool(trans_model, test_ids, lambda vid, i: fixed_feats(all_raw_feats[vid], top_k, 0))
    test_fixed1 = eval_pool(trans_model, test_ids, lambda vid, i: fixed_feats(all_raw_feats[vid], top_k, max(1, (WINDOW//top_k)//2)))
    test_tsplus = eval_pool(trans_model, test_ids, lambda vid, i: argmax_feats(all_raw_feats[vid], top_k, all_score_z[vid]))
    ...
    test_sc_a   = eval_pool(trans_model, test_ids, lambda vid, i: argmax_feats(all_raw_feats[vid], top_k, all_sc_z[vid]))
    test_sc_s   = eval_pool(trans_model, test_ids, lambda vid, i: sc_stoch_feats(all_raw_feats[vid], all_sc_z[vid], WINDOW, STRIDE, top_k, TEMPERATURE, np.random.default_rng(2000+rng_base*10+i)), n_draws=N_EVAL_DRAWS)
    print(f"RESULT transsvnet_allframe_{DATASET} window={WINDOW} topk={top_k} seed={SEED}  random={test_random:.4f} fixed0={test_fixed0:.4f} fixed1={test_fixed1:.4f} TSplus={test_tsplus:.4f} MGSampler={test_mg:.4f} SCSampler_argmax={test_sc_a:.4f} SCSampler_stoch={test_sc_s:.4f}")
