"""TeCNO trained on ALL frames in the window (full mean-pool), evaluated at
inference under top_k=8 selection strategies: random, fixed0, fixed1(mid),
MobileNet-TS+ (argmax), MGSampler (deterministic), SCSampler-style (argmax +
stochastic).
usage: python grid_tecno_allframe.py <seed>
"""
import sys, random
...
N_EVAL_DRAWS = 5
CKPT_OUT = Path(f"/tmp/ckpt_tecno_allframe_cholec80_seed{SEED}.pt")
...
print(f"RESULT tecno_allframe seed={SEED}  random={test_random:.4f} fixed0={test_fixed0:.4f} fixed1={test_fixed1:.4f} TSplus={test_tsplus:.4f} MGSampler={test_mg:.4f} SCSampler_argmax={test_sc_a:.4f} SCSampler_stoch={test_sc_s:.4f}")
