"""Fine-tune MobileNet-TS+ on AutoLaparo/HeiChole starting from the
Cholec80-trained backbone+trunk+flow_head (not from generic ImageNet
weights) -- only the classification head is freshly initialized, since
phase-label semantics may differ across datasets even when the class count
happens to match. Fewer epochs should suffice since the starting point
already encodes surgical-video-relevant low-level and motion cues, not just
generic natural-image features.
usage: python finetune_mobilenet_multitask_from_cholec80.py <dataset> <epochs>
"""
import sys, time
...
OUT_ROOT = Path(f"{BASE}/{DATASET}/mobilenet_multitask_ft_score")
DEVICE = "cuda"
BATCH = 64
...
print(f"initialized backbone+trunk+flow_head from Cholec80 checkpoint (freeze_before={FREEZE_BEFORE}); cls_head is fresh")

opt = Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-4, weight_decay=1e-5)

print(f"=== [{DATASET}] fine-tuning from Cholec80 init, Train:{len(train_ids)}, epochs={EPOCHS} ===")
for epoch in range(1, EPOCHS + 1):
    ...
    print(f"  epoch {epoch}/{EPOCHS}  cls_loss={total_cls/n_frames:.4f}  flow_mse={total_flow/n_frames:.4f}  train_acc={n_correct/n_frames:.4f}  ({time.time()-t0:.0f}s)", flush=True)

model.eval()
for p in model.parameters(): p.requires_grad = False

print(f"\n=== [{DATASET}] extracting scores ===")
...
print(f"[{DATASET}] saved fine-tuned scores for {len(all_ids)} videos to {OUT_ROOT}")
